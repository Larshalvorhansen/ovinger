...
class Resource(T)
{
    ...
    T allocate(int prioritet)
    {
        mtx.wait();
        if (opptatt)
        {
            numWaiting[prioritet]++;
            mtx.notify();
            sems[prioritet].wait();
            numWaiting[prioritet]--;
        }
...
void main()
{
    ...
    auto cfgs = [
        ...
        ResourceUserConfig(7, 0, 11, 1), ResourceUserConfig(8, 1, 11, 1),

        ResourceUserConfig(0, 1, 25, 3), ResourceUserConfig(6, 0, 26, 2),
        ResourceUserConfig(7, 0, 26, 2), ResourceUserConfig(1, 1, 26, 2),
        ResourceUserConfig(2, 1, 27, 2), ResourceUserConfig(3, 1, 28, 2),
        ResourceUserConfig(4, 1, 29, 2), ResourceUserConfig(5, 1, 30, 2),
    ];
